...
export const confirm_reset_password = createAsyncThunk(
    ...
    async (user, thunkAPI) => {
        try {
            return await authService.confirm_reset_password(user);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const authSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(confirm_reset_password.pending, (state) => {
                state.isLoading = true;
            })
            .addCase(confirm_reset_password.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
            })
            .addCase(confirm_reset_password.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.message = action.payload;
            });
